...
class Program
{
  ...
  static void Main(string[] args)
  {
    ...
    if (faturamento == null)
    {
      Console.WriteLine("Erro ao deserializar o JSON.");
      return;
    }

    decimal menorFaturamento = faturamento.Where(x => x.valor > 0).Min(x => x.valor);
    decimal maiorFaturamento = faturamento.Max(x => x.valor);
    decimal mediaMensal = faturamento.Where(x => x.valor > 0).Average(x => x.valor);
    int diasAcimaMedia = faturamento.Count(x => x.valor > mediaMensal);
